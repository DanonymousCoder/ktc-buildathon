...
3. Open a local PDF with Chrome, for example `file:///Users/<you>/Documents/sample.pdf`.
4. Open the popup.
...
1. Open a PDF tab and confirm FlowTrakka is tracking.
...
- `daily_logs[<today>].total_seconds` increments by 60 while the PDF remains active and the user is not idle.
- No service-worker error appears on the extension card.

## Distraction Test

1. Open a PDF tab and confirm tracking.
2. Switch to a non-PDF tab, such as YouTube.
3. Wait one heartbeat interval.

Expected:
- `trackingState.status === "inactive"`.
- `daily_logs[<today>].total_seconds` does not increment while the non-PDF tab is active.

## Midnight Roll-over

1. Open a PDF and confirm tracking.
